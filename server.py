...
def amass(
    ...
    Args:
        subcommand: Specify the Amass operation mode - 'enum' for subdomain enumeration and network mapping, or 'intel' for gathering intelligence about target domains from various sources
        ...
        enum_type: Enumeration approach type - 'active' includes DNS resolution and potential network interactions, 'passive' only uses information from third-party sources
        enum_brute: Whether to perform brute force subdomain discovery (true/false)
        enum_brute_wordlist: Path to custom wordlist file for brute force operations (e.g., '/path/to/wordlist.txt')
    """
    ...
    try:
        ...
        if result.returncode != 0:
            return f"Amass exited with code {result.returncode}. Output: {output}. Args: {' '.join(amass_args)}"
